...
def ip_url_logger(line):
    #Checks needed
    tempLine = line.split(' ')
    # print(tempLine)
    if tempLine[0][0].isdigit():
        if tempLine[0] not in ip_dict:
            ip_dict[tempLine[0]] = 1
        else:
            ip_dict[tempLine[0]] = ip_dict[tempLine[0]] + 1

    if tempLine[6][0] == '/' or 'h':
        if tempLine[6] not in url_dict:
            url_dict[tempLine[6]] = 1
        else:
            url_dict[tempLine[6]] = url_dict[tempLine[6]] + 1
